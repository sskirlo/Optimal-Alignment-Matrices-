...
c   switches based on width, need to specify switch for all possiblities
	GO TO (14,14,15,16,10001,10002),W
C
...
1145	II=0
	DO 11451 VAR=1,N
	II=TP(A(VAR,0))*2**(N-VAR)+II
11451   CONTINUE
c        WRITE(*,*) 'T',II
        GO TO 113
1146	II=0
	DO 11461 VAR=1,N
	II=TPX(A(VAR,0))*2**(N-VAR)+II
11461   CONTINUE
c	WRITE(*,*) 'HT ',II
        GO TO 113
1147	II=0
	DO 11471 VAR=1,N
	II=TP(A(VAR,0))*2**(VAR-1)+II
11471   CONTINUE
c	WRITE(*,*) 'VT ',II
        GO TO 113
1148	II=0
	DO 11481 VAR=1,N
	II=TPX(A(VAR,0))*2**(VAR-1)+II
11481   CONTINUE
c	WRITE(*,*) 'VHT ',II
	K=-1
	GO TO 113
...
c end testing code
	...
	PHI=(Base**7)*IM+(Base**6)*IM+(Base**5)*IM+(Base**4)+(Base**3)*IM
	THE=(Base**6)*IN+(Base**4)*IN
	I=(Base**7)*A(1,0)+(Base**6)*A(2,0)+(Base**5)*A(3,0)
...
c only sets k=-1 if has gone through all possibilities, then k will equal 0 at this point if finds lexographically least, so can go on to autocorrelation test
        IF(K.EQ.0)	GO TO 777
	GO TO (1123,1143,1122,1142,1145,1146,1147,1148),K
111	GO TO 2
C
...
c        If (W.NE.(5)) GO TO 338
c        If (A(3,0).NE.(0)) GO TO 338
c        If (A(2,0).NE.(0)) GO TO 338
c        If (A(1,0).NE.(0)) GO TO 338
c        write (*,*) 'A(1,0)', A(1,0)
c        write (*,*) 'A(2,0)', A(2,0)
c        write (*,*) 'A(3,0)', A(3,0)
c        write (*,*) 'A(4,0)', A(4,0)
...
c mask is used to pick up overflow bits, place so that at end of 8 bit allocation, so 7, 15, 23, etc. 
	...
	DO 3000 J=1,W-D
        KK1=(Base**6)*COR(A(J,0),A(J+D,0))
     &	+(Base**5)*( COR(A(J,1),A(J+D,-1) )-1)
     &  +(Base**4)*(COR(A(J,2),A(J+D,-2))-2)
c     &  +(Base**3)*(COR(A(J,3),A(J+D,-3))-3)
c     &	+(Base**2)*(COR(A(J,4),A(J+D,-4))-4)
c     &  +(Base**1)*(COR(A(J,5),A(J+D,-5))-5)
c     &	+(COR(A(J,6),A(J+D,-6))-6)
	R1=R1+KK1
3000	CONTINUE

	U1=(Base**6)*V+(Base**5)*V+(Base**4)*V
	RU1=R1+U1
	RC1=AND(RU1,MASK1)
...
334	IF (RC1.GT.0)	GO TO 2
	L1=(Base**6)*VV+(Base**5)*VV+(Base**4)*VV
	RL1=R1-L1
	RC3=AND(RL1,MASK1)
...
707     DO 4444 D=0,W-1
	...
	R(D,TAU)=0
	DO 2222 I=1,W-D
	R(D,TAU)=R(D,TAU)+((COR(A(I,-TAU),A(I+D,TAU))-ABS(TAU))-N)
2222	CONTINUE
3333	CONTINUE
4444	CONTINUE
C
...
1110	FORMAT (5X,I3,5X,I3,5X,I3,5X,I3,5X,I3)
	GO TO 2
19      GO TO 2
c19	WRITE (M,1111) (A(I,0),I=1,W)
c1111	FORMAT (5X,I3,5X,I3,5X,I3,5X,I3)
c	GO TO 2
c18      GO TO 2
18	WRITE (M,1112) (A(I,0),I=1,W),(R(0,TAU),TAU=0,N-1),
     *	((R(D,TAU),TAU=1-N,N-1),D=1,W-1)
1112	FORMAT (5X,I3,5X,I3,5X,I3,24X,33I3)
	GO TO 2
17      GO TO 2
c17	AB=(2**N)-1-A(2,0)	
c	IF (AB.LT.A(2,0)) 	GO TO 5555
