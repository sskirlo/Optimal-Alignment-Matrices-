...
c REMMEBER TO CHANGE BACK PARTS SO IS FAST FOR -1,1 CAN REMOVE STUFF WITH PASS SWITCHES, CHANGE BACK SO WILL AUTOMATICALLY HAVE 0,0,0 IN LEFT CORNER
...
c integer*8 are new 64 bit words which will be able to handle 7 by 7 matrices, not available when code was originally written
	...
	IMPLICIT INTEGER(KIND=8) (A-Z)
     ...
     &  KK1, KK2, COUNT1, SUM1, SWIT
	DIMENSION COR(0:127,0:127),IWT(0:127),A(20,-7:7),JJ(7),
     &  X(0:127),H(4),T(4),R(0:7,-6:6), TP(0:127), TPX(0:127)
	M=1
	COUNT1=0
...
c	T(1)=88
	...
	SWIT=0
c	if(H(1).EQ.(13)) GOTO 11111
c	GOTO 11115
c11111	if(H(2).EQ.(4)) GOTO 11112
c	GOTO 11115
c11112	if(T(2).EQ.(110)) GOTO 11113
c	GOTO 11115
c11113	if(T(1).EQ.(88)) GOTO 11114
c	GOTO 11115
c11114   write(*,*) 'Have match'
c	SWIT=1

c   switches based on width, need to specify switch for all possiblities
11115	GO TO (14,14,15,16,10001,10002,10003),W
C
...
c if these conditions satisfied should automatically pass, for other cases either doesn't matter if pass=1 (b/c w=lmax, will backtrack anyways), or because tailcheck will properly set pass
	IF((W.LT.LMAX).AND.(NH.NE.NT)) PASS=1
	IF(PASS.EQ.(1)) GO TO 2

c   loop starts here for tail check and then middle check
3	V=(2**7-1)-((W-D)*N+CORMAX)
	VV=((W-D)*N-CORMAX)
	S=1
        Base2=2**7

c if W is less than LMAX and odd, no point in doing check, make pass tailcheck automatic
...
c  this statement actuallty does nothing but depending on whether or not included, program will or will not compile correctly

c	WR=0
c	IF((H(1).EQ.0).AND.(T(1).EQ.30).AND.(W.EQ.2)) WR=1
c	IF(WR.EQ.0) GO TO 2
c	write(*,*) 'pass',PASS	

c	IF(H(1).EQ.(1)) GO TO 5555

2	IF (PASS.EQ.(0)) GO TO 6
